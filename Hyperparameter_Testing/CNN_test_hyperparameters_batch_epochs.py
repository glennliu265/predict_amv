...
# -*- coding: utf-8 -*-
"""
CNN_test_hyperparameters_batch_epochs

Testing batchsize and epochs
...
param1 = [16,32,64,128] # Indicate batch sizes here
param2 = [1,5,10,15]      # Indicate max training epochs here
...
train_loss_grid = np.zeros(gridsize)
test_loss_grid  = np.zeros(gridsize)
...
for i in range(np1): # Loop for Learning Rate
    ...
    for j in range(np2): # Loop for Weight Decay
        ...
        train_loss_grid[i,j] = np.array(trainloss).min(2).squeeze() # Take minum of each epoch
        test_loss_grid[i,j]  = np.array(testloss).min(2).squeeze()
